...
def get_purchase_invoice():
    ...
    doc_list = frappe.get_all('Purchase Invoice', filters={'docstatus': 1, "update_stock":1, 'is_return': 0}, fields=['*'])
    list_of_purchases= []
    for doc in doc_list:
                ...
                for row in document.items:
                    if 'Input GST Out-state' in document.taxes_and_charges:
                        doc_json ={
                            ...
                            "BatchNo": row.batch_no if row.batch_no else "",
                            "Quantity": str(row.qty) if row.qty else "",
                            "Rate": str(row.net_rate) if row.net_rate else "",
                            "Discount": "",
                            "Amount": str(row.net_amount) if row.net_amount else "",
                            "OrderNo": "",
                            "OrderDate": "",
                            "TrackingNo": "",
                            "TrackingDate": "",
                            ...
                            "GstOvrdnTypeofsupply":row.item_group if row.item_group else "",
                            "GstHsnName":row.gst_hsn_code if row.gst_hsn_code else "",
                            "GstHsnDescription":row.description if row.description else "",
