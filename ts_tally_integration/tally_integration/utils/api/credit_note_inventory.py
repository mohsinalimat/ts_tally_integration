...
@frappe.whitelist()
def credit_note_inv(company=None):
    if company==None:
    ...
    if company_number:
        ...
        if tally_settings[0]['stock'] == 'Non-Inventory':
            return 'Company is Inventory, but Request is Non Inventory'

        company_name = tally_settings[0]['company_name']

        credit_doc = frappe.db.get_list('Sales Invoice',filters={'company':company_name,'is_return':1, 'update_stock':1, 'docstatus':1},fields=['*'])
